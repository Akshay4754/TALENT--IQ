// ============================================================
// FULLY LAZY VERCEL SERVERLESS FUNCTION
// All imports are dynamic to prevent module-level crashes
// ============================================================
...
async function buildApp() {
  // Dynamic imports - if any package crashes on import, we catch it
  const [
    expressMod,
    inngestExpressMod,
    corsMod,
    ...
    import("mongoose"),
    import("stream-chat"),
    import("@stream-io/node-sdk"),
    import("inngest"),
    import("@clerk/express"),
  ]);

  const express = expressMod.default;
  const { serve } = inngestExpressMod;
  const cors = corsMod.default;
  const mongoose = mongooseMod.default;
  const { StreamChat } = streamChatMod;
  const { StreamClient } = streamNodeMod;
  const { Inngest } = inngestMod;
  const { clerkMiddleware, requireAuth, clerkClient } = clerkMod;
  ...
  const deleteUserFromDB = inngest.createFunction(
    ...
    async ({ event }) => {
      ...
      try {
        await getChatClient().deleteUser(id);
      } catch (e) {
        console.error(e);
      }
    }
...
// ============================================================
// EXPORT: Lazy handler that builds app on first request
// ============================================================
